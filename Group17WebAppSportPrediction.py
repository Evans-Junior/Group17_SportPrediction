...
with open("sport_model_h.sav", "rb") as file:
  stats,scaler, loaded_model = pickle.load(file)
  ...
  #   t_critical = 2.0  # For a 95% confidence interval with a large degrees of freedom
  #   # You can calculate t-critical from a t-distribution table for your specific degrees of freedom
...
def predict_with_interval(Name, original_overall, input_data, model=loaded_model, scaler=scaler, confidence_level=0.95):
  ...
  alpha = 1 - confidence_level
  t_critical = stats(1 - alpha / 2, df=degrees_of_freedom)
  margin_of_error = t_critical * se_regression
